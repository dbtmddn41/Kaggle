...
class EventDetectionAveragePrecision(keras.metrics.Metric):
    def __init__(self, name="edAP", **kwargs):
        ...
        # self.total_batchs = self.add_weight(name='total_samples', initializer='zeros', dtype='int32')
    ...
    def update_state(self, y_true, y_pred, sample_weight=None):
        ...
        if self.on:
            y_true = y_true.numpy()
            y_pred = y_pred.numpy()
            batch, steps, _ = y_true.shape
            series_id = np.repeat(np.arange(batch), steps * 2).astype('str')
            event = np.tile(np.repeat(['0', '1'], steps), batch).astype('str')
            step = np.tile(np.arange(steps), batch * 2).astype(float)

            submission = pd.DataFrame({
                'series_id': series_id,
                'event': event,
                'score': y_pred[:, :, :2].transpose((0,2,1)).reshape(-1),
                'step': step,
            })
            series_id, event, step = y_true[:, :, :2].nonzero()
            solution = pd.DataFrame({
                'series_id': series_id.astype('str'),
                'event':event.astype('str'),
                'step': step,
            })
            self.score.assign_add(event_detection_ap(solution, submission))
            # self.total_batchs.assign_add(1)

    def result(self):
        return self.score #/ tf.cast(self.total_batchs, tf.float32)
...
def event_detection_ap(
    ...
    ap_table = (
        ...
        # 오류나서 교체. (더 빠른 코드라 오히려 좋아.)
        detections_matched[np.isin(detections_matched['event'], event_classes)]
